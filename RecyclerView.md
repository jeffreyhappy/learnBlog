### Recycler
Recycler的责任是为重用管理丢弃和解绑定item View,
scrapped(报废) 视图:
是那些继续绑定在它的父recyclerView中,但是已经被标记为移除或者重用的视图
通常,通过LayoutManager使用Recycler是用来替adapter的data获取视图来代表对应的data在给定的位置或者item id
如果这个视图被认为"dirty"就会被重用,这个adapter就会被告诉重新绑定它
如果不是,这个可以被LayoutManager不需要额外操作而快速重用
还没有调用requested layout 的被清除的视图可能被LayoutManager重新定位并且不用重新测量
...
public final class Recycler {
        ...
        //mUnmodifiableAttachedScrap 是不可修改的VH 列表
        ...
        //RecycledViewPool可以在多个RecyclerView之间共享View,通过setRecycledViewPool来设置,没有就会自己创建一个
        RecycledViewPool mRecyclerPool;

        //ViewCacheExtension是一个视图帮助类,用来帮助开发者管理视图缓存
